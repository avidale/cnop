...
for(it = start_iter-1; it < 50000; it++){
	...
	ready_boot = 0
	for (it_boot = 0; it_boot < times_per_iter; it_boot++) {
		it, it_boot
		if (bootstrap) {
			// bootstrap
			rseed(42 * 42 + it * times_per_iter + it_boot)
			boot_indices = runiformint(100, 1, 1, 100)
		} else {
			boot_indices = runningsum(J(n,1,1))
		}
		...
		ready_boot = ready_boot + 1
		if (ready_boot >= times_per_iter) {
			break
		}
	}
